...
export default function SegmentFilteredContent({
  ...
  const filtered = activeType
    ? companies.filter(c => c.aiTypes.includes(activeType))
    : companies;

  const fundedFiltered = filtered.filter(c => c.funding > 0);
  // When filtering by AI type, show all matching companies; default view shows funded only
  ...
  const activeLabel = activeType
    ? aiTypeCounts.find(t => t.type === activeType)?.label
    : null;
    ...
    <>
      ...
      {aiTypeCounts.length > 0 && (
        <section className="mb-10">
          <div className="mb-5">
            <p className="label-refined text-accent mb-2">Technologies</p>
            <div className="flex items-center gap-6">
              <h2 className="headline-sub whitespace-nowrap">AI Technologies Used</h2>
          ...
          <div className="flex flex-wrap gap-2.5">
            {aiTypeCounts.map(t => {
              ...
              return (
                <button
                  key={t.type}
                  onClick={() => setActiveType(isActive ? null : t.type)}
      ...
      <section className="mb-10">
        <div className="mb-5">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1 h-px bg-gradient-to-r from-border/50 to-transparent" />
            <Link
              href={`/directory?segment=${segmentSlug}`}
